...
public:
    AnalogyEngine(const AttributeWeights& w) : weights(w) {
        ...
        database.push_back({"red wine", "dry", "red", "France", 13.5, true, true});
        database.push_back({"green tea", "bitter", "green", "China", 0.0, false, false});
        database.push_back({"vodka", "neutral", "clear", "Russia", 40.0, true, true});
        database.push_back({"orange juice", "sweet", "orange", "Brazil", 0.0, false, false});
        database.push_back({"atay", "3assir dyal 5izou", "Monada", "Morocco", 0.0, false, false}); 
    }
